...
class SDNE(object):
        ...
        if batch_size >= self.node_size:
            ...
            for epoch in tqdm(range(initial_epoch, epochs)):
                ...
                for i in range(steps_per_epoch):
                    ...
                    batch_losses = self.model.train_on_batch(x=inp, y=inp)
                    # batch_losses = self.model.train_on_batch(inp)
                    losses += batch_losses
    ...
    def get_embeddings(self):
        self._embeddings = {}
        embeddings = self.emb_model.predict(self.A.todense(), batch_size=2048)
